...
import { parseAdbDevices, parseAdbPackages, parseAvdList, parseGradleOutput } from '../backend/parsers/android.js';
import {
  buildAdbDevicesArgs,
  buildAdbConnectArgs,
  ...
  buildAvdListArgs,
  buildEmulatorArgs,
} from '../backend/commands/android.js';
...
export async function buildProject(opts) {
  ...
  const { stdout, stderr } = await execa(gradlew, [opts.task ?? 'assembleDebug'], {
    cwd: opts.projectDir,
    timeout: 300000,
  });
...
export async function listAvds() {
  const { stdout } = await execa('avdmanager', buildAvdListArgs(), { timeout: 15000 });
  return parseAvdList(stdout);
}

export async function launchEmulator(opts) {
  return execa('emulator', buildEmulatorArgs(opts), { timeout: 0, detached: true });
}

export const INSTALL_HINT = 'Install Android SDK: see developer.android.com/studio or brew install --cask android-studio';
